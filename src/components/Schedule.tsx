...
const day1Schedule: ScheduleItem[] = [
  ...
  { id: 'keynote1', time: '10:00 AM - 10:30 AM', title: <>Keynote Talk 1:{' '}<span className="quote-normal">Technology as a Catalyst for Change – Innovating with Purpose</span></>, details: 'Dive into the world of AI Ethics...' },
  { id: 'keynote2', time: '10:30 AM - 11:00 AM', title: <>Keynote Talk 2:{' '}<span className="quote-normal">The Role of Youth in Driving Tech-Driven Social Impact</span></>, details: 'Our second keynote focuses on leveraging blockchain...' },
  { time: '11:00 AM - 11:15 AM', title: 'Tea Break' },
  { id: 'ideacraft', time: '11:15 AM - 12:30 PM', title: <>IdeaCraft:{' '}<span className="quote-normal">It All Starts With a ‘What If?’</span></>, details: 'An interactive workshop where participants will learn...' },
  { time: '12:30 PM - 1:00 PM', title: 'Ice Breaking Session' },
  { time: '1:00 PM - 2:00 PM', title: 'Lunch and Networking Session' },
  { id: 'panel', time: '2:00 PM - 3:00 PM', title: 'Panel Discussion', details: 'Industry leaders discuss the role of corporate social responsibility...' },
  { id: 'benefits', time: '3:00 PM - 3:30 PM', title: 'Benefits of IEEE membership', details: 'Discover the vast array of resources...' },
  { time: '3:30 PM - 4:00 PM', title: 'Closing Remarks' },
  { time: '4:00 PM - 4:30 PM', title: 'Tea Break' },
  { id: 'hackathon', time: '5:00 PM - 6:00 PM', title: 'Inauguration and Commencement of Hackathon', details: 'The 24-hour hackathon officially begins! ...' },
    ...
    return scheduleData.map((item, index) => (
      <div className="schedule-item" key={`${item.time}-${index}`}>
        ...
        <div className="schedule-details">
          <h4 onClick={() => item.id && handleToggle(item.id)} className={item.id ? 'clickable' : ''}>
            {item.title}
            {item.id && <i className={`fas fa-chevron-down toggle-icon ${openItemId === item.id ? 'open' : ''}`}></i>}
          </h4>
          {item.id && openItemId === item.id && (<div className="dropdown-content"><p>{item.details}</p></div>)}
        </div>
      </div>
    ));
  };
    ...
    <section id="schedule">
      <div className="container">
        ...
        <div className="schedule-tabs" data-aos="fade-up" data-aos-delay="200">
          <button className={`tab-btn ${activeTab === 'day1' ? 'active' : ''}`} onClick={() => setActiveTab('day1')}>Day 1 (Nov 8)</button>
          <button className={`tab-btn ${activeTab === 'day2' ? 'active' : ''}`} onClick={() => setActiveTab('day2')}>Day 2 (Nov 9)</button>
        </div>
